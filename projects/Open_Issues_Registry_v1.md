...
### KI-01 — CCA1 v3.2 Output Field Names Unverified

**Severity:** CRITICAL  
**Affects:** Connections C1, C2, C5 — the three connections that originate from CCA1 and fan out to GAP17, CCW68 Phase 0D, and ARID14 respectively. Also affects schema parser design for any table that receives CCA1 output.

**What the problem is:** The CCA1 v3.2 prompt file was never shared as a standalone file during config development. Field names in the C1, C2, and C5 translation maps in WorkflowConfig v1.8 were inferred from inline session descriptions, not verified against the actual prompt output. If CCA1's actual output labels differ from the inferred labels, every connection originating from CCA1 will silently extract the wrong fields or miss data.

**Why it's critical now:** This is the workflow entry point. Every orchestrated run begins at CCA1. An unverified entry point is the highest-risk unresolved gap in the entire system. Any automated agent that executes CCA1 without verified field maps will propagate bad data to every downstream step.

**Action required:** Provide the CCA1 v3.2 prompt file in a session. Run a verification pass against the C1, C2, and C5 translation maps in WorkflowConfig v1.8. Correct any field name mismatches. Update DEPENDENCY_MAP.md and close KI-01 in the GitHub repo.  
**Estimated time:** 30–60 minutes.  
**Owner:** Noah Gamer (provide file) → Claude (verification pass)  
**Status:** Open

---

### KI-02 — ARV20 Version Mismatch in Config Registry

**Severity:** CRITICAL  
**Affects:** Connection C6 (Article Audit to Article Refresh), Connection C9 (Article Refresh to SWA Optimization). Also affects any agent that reads the Prompt Registry to determine which gate structure to apply.

**What the problem is:** WorkflowConfig v1.8 Prompt Registry lists ARV20 at version 2.0. The live prompt file is now version 2.5 — two versions ahead. The config's Version Compatibility Matrix references v2.0 behaviors. ARV20 v2.1 already changed Gate 1 behavior (removed Microsoft 365 MCP dependency, replaced SharePoint Phrase Log integration with operator-provided prior log review). The delta between v2.1 and v2.5 is not yet documented in this project.

**Why it's critical now:** The Jira pipeline status workflow for the refresh path was designed against the architecture spec, which itself references the config's version registry. An automation layer reading the config will apply v2.0 gate logic to a prompt running v2.5 behavior. This is a hard failure mode.

**Action required:** Update the Prompt Registry in WorkflowConfig v1.8 to reflect ARV20 v2.5. Document the gate structure changes between v2.0 and v2.5. Verify Connection C6 and C9 translation maps against the v2.5 gate structure. Update DEPENDENCY_MAP.md.  
**Owner:** Noah Gamer  
**Status:** Open
...
**Affects:** Project 2 Addendum (Tool Services Layer), Phase 0 of build sequence, all agent build decisions involving Playwright and docx skill

**What the problem is:** The Project 2 architecture does not specify whether agents run on Claude Code on a VM/server or via Databricks API calls. The Tool Services Layer addendum defines three services (URL Verification, Document Generation, Orchestration Runtime) that have different implementation paths depending on which environment is chosen. This decision must be made before Phase 0 of the build sequence, which is the prerequisite gate for Phase 1.
...
**What the problem is:** The Project 3 scoping document describes a Topic Intelligence Database (Problem A) with approximately 5 tables covering topic records, performance history, cluster health, qualification scores, and cannibalization mapping. The SQL schema v2.1 produced March 22 covers 64 tables across 11 schemas — incorporating both Problem A (topic intelligence, pre-production planning) and Problem B (production intelligence, operational telemetry from gate-level pass/fail data, citation maps, RPS component scores, artifact tracking). The AI Engineer cannot build the correct schema without a formal decision on which scope to build.
...
**What the problem is:** Company-managed Jira projects with shared workflow schemes are required for the pipeline (confirmed March 23 session). Shared workflow schemes require a specific Atlassian plan tier. The current tier has not been confirmed as supporting this feature across multiple projects.
...
**Severity:** HIGH  
**Affects:** Project 3 database maintenance skill, qualification_scores table, all recommended action logic
...
**What the problem is:** The post-publish 90-Day Review Agent queries GSC data and Databricks analytics data. The Project 3 database lives in Databricks Unity Catalog. Confirm that the Databricks SQL MCP connection has SELECT and INSERT/UPDATE access to the relevant tables in the GenAI workspace, and that GSC property permissions cover all 85+ university domains.
...
### Q12 — Tool Services Layer Ownership
...
### Q2 — GSC Property Access Coverage

**Severity:** MEDIUM  
**Affects:** Project 3 weekly performance refresh, 90-Day Review Agent

**What the problem is:** The weekly performance refresh queries GSC for all published article URLs. If the GSC MCP connection does not have read access to all 85+ university domain GSC properties, the performance refresh will be incomplete.

**Action required:** Confirm which GSC properties the current connection has access to. If not all 85+ domains, determine the process for adding missing properties.  
**Owner:** Noah Gamer + IT/Systems  
**Status:** Open

---

### Q6 — Existing ARID14 Workbook Consolidation Decision

**Severity:** MEDIUM  
**Affects:** Project 3 initial population — seed data strategy

**What the problem is:** Multiple ARID14 workbooks exist for different programs and dates (confirmed fixtures: FIT MBA General, A-State MSN Nurse Admin). A decision is needed on whether to import all existing workbooks as seed data or start fresh with a GSC-only baseline.

**Action required:** Decision: import existing ARID14 workbooks as seed data (faster, richer starting state) or GSC-only baseline (cleaner, more consistent). Recommendation: import existing workbooks for confirmed fixture programs; GSC-only for all others.  
**Owner:** Noah Gamer  
**Status:** Open
...
**Action required:** Schedule sign-off conversations. Provide `Project1_EditorialInterface_Risepoint.docx` as the primary brief.  
**Owner:** Noah Gamer  
**Status:** Open
...
## Pre-Phase 4 Issues — OC Schema (Must Resolve Before Parser Build)

These issues were identified in the March 23 technical review of the SQL schema against the OC system documentation. They must be resolved before the Phase 4 parser build begins.
...
| OC-2 | ARV20 version in schema | Schema references ARV20 v2.1; live file is v2.5. Gate structure in `step5_refresh` tables must be verified against v2.5. | Noah Gamer (KI-02 resolution covers this) |
...
| OC-5 | ARID14 tab-to-column mapping | ARID14 Excel tab-to-column mapping is not documented. Parsers for ARID14 data cannot be written without it. Recommended: add to Order #0 before Phase 1 deployment. | Noah Gamer (provide ARID14 workbook structure) |

---

## Resolved Issues — Archive

*No issues resolved as of March 25, 2026. This section will be populated as issues close.*
...
*Document generated March 25, 2026. To be committed to `/projects/` in github.com/ngamer/workflow-designs. Update this document whenever an issue is resolved or a new issue is identified.*
